...
class GoogleSheetsAuthService(private val context: Context) {
    ...
    private fun getSheetsService(): Sheets? {
        return try {
            ...
            Log.d("GoogleSheetsAuth", "Resource ID: $resourceId")
            
            if (resourceId == 0) {
                Log.e("GoogleSheetsAuth", "service_account.json not found in raw resources")
                return null
            }
    ...
    private suspend fun ensureDailySheetExists(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val response = sheetsService.spreadsheets().batchUpdate(spreadsheetId, batchUpdateRequest).execute()
                ...
                if (response.replies?.isNotEmpty() == true) {
                    ...
                    val headerValues = listOf(listOf("Timestamp", "Title", "Description", "Image URL"))
    ...
    suspend fun appendValues(
        ...
        return withContext(Dispatchers.IO) {
            try {
                Log.d("GoogleSheetsAuth", "Starting appendValues - spreadsheetId: $spreadsheetId, range: $range")
                ...
                for (currentRange in rangesToTry) {
                    try {
                        ...
                        if (response != null && response.updates != null) {
                            Log.d("GoogleSheetsAuth", "Successfully appended ${response.updates.updatedRows} rows to range: $currentRange")
                            return@withContext Result.success(true)
                        } else {
                            Log.w("GoogleSheetsAuth", "Response is null or updates is null for range: $currentRange. Response: $response")
                        }
                    } catch (e: Exception) {
                        Log.w("GoogleSheetsAuth", "Failed to append to range $currentRange: ${e.message}")
